...
namespace SteamAuth
{
    public class UserLogin
    {
        ...
        public LoginResult DoLogin()
        {
            ...
            if (cookies.Count == 0)
            {
                Uri url = new System.Uri("http://steamcommunity.com");
            ...
            byte[] encryptedPasswordBytes;
                var provider = AsymmetricKeyAlgorithmProvider.OpenAlgorithm(AsymmetricAlgorithm.RsaPkcs1); // create provider
                var passwordBytes = Encoding.GetEncoding("ASCII").GetBytes(this.Password); //get bytes from password
                RSAParameters rsaParameters = new RSAParameters(); //create parameters
                ...
                provider.ImportParameters(rsaParameters); //import parameters
                var key = provider.CreateKeyPair(1024); //generate random key
                encryptedPasswordBytes = CryptographicEngine.Encrypt(key,passwordBytes); //encrypt password
