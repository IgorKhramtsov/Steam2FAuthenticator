...
namespace SteamAuth
{
    public class AuthenticatorLinker
    {
        ...
        public FinalizeResult FinalizeAddAuthenticator(string smsCode)
        {
            ...
            while (tries <= 30)
            {
                if (postData.ContainsKey("authenticator_code"))
                    postData["authenticator_code"] = LinkedAccount.GenerateSteamGuardCode();
                else
                    postData.Add("authenticator_code", LinkedAccount.GenerateSteamGuardCode());

                if (postData.ContainsKey("authenticator_time"))
        ...
        public static string GenerateDeviceID()
        {
            ...
            NetFxCrypto.RandomNumberGenerator.GetBytes(randomBytes);

            var hasher = WinRTCrypto.HashAlgorithmProvider.OpenAlgorithm(HashAlgorithm.Sha1);
            byte[] hashedBytes = hasher.HashData(randomBytes);
