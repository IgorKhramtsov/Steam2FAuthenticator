...
namespace SteamAuth
{
    public class AuthenticatorLinker
    {
        ...
        public async Task<LinkResult> AddAuthenticator()
        {
            ...
            if (!hasPhone)
                if (!await _addPhoneNumber().ConfigureAwait(true))
            ...
            string response = await SteamWeb.MobileLoginRequest(APIEndpoints.STEAMAPI_BASE + "/ITwoFactorService/AddAuthenticator/v0001", "POST", postData).ConfigureAwait(true);
        ...
        public async Task<FinalizeResult> FinalizeAddAuthenticator(string smsCode)
        {
            ...
            if (!String.IsNullOrEmpty(this.PhoneNumber) && !await this._checkSMSCode(smsCode).ConfigureAwait(true))
            {
            ...
            while (tries <= 30)
            {
                ...
                string response = await SteamWeb.MobileLoginRequest(APIEndpoints.STEAMAPI_BASE + "/ITwoFactorService/FinalizeAddAuthenticator/v0001", "POST", postData).ConfigureAwait(true);
        ...
        private async Task<bool> _checkSMSCode(string smsCode)
        {
            var postData = new NameValueCollection();
            ...
            string response = await SteamWeb.RequestAsync(APIEndpoints.COMMUNITY_BASE + "/steamguard/phoneajax", "POST", postData, _cookies).ConfigureAwait(true);
        ...
        private async Task<bool> _addPhoneNumber()
        {
            var postData = new NameValueCollection();
