    ...
    public class UserLogin
    {
        ...
        public async Task<LoginResult> DoLogin()
        {
            ...
            if (cookies.Count == 0)
            {
                ...
                await SteamWeb.MobileLoginRequest("https://steamcommunity.com/login?oauth_client_id=DE45CD61&oauth_scope=read_profile%20write_profile%20read_client%20write_client", "GET", null, cookies, headers).ConfigureAwait(true);
            }

            postData.Add("username", this.Username);
            response = await SteamWeb.MobileLoginRequest(APIEndpoints.COMMUNITY_BASE + "/login/getrsakey", "POST", postData, cookies).ConfigureAwait(true);
            ...
            response = await SteamWeb.MobileLoginRequest(APIEndpoints.COMMUNITY_BASE + "/login/dologin", "POST", postData, cookies).ConfigureAwait(true);
            ...
            else { this.RequiresCaptcha = false; this.CaptchaGID = ""; }
